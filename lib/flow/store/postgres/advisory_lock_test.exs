defmodule Ariadne.Flow.Store.Postgres.AdvisoryLockTest do
  ...
  describe "advisory lock keys" do
    test "are distinct for inputs that only differ in one part" do
      ...
      keys = Map.new(inputs, fn {label, parts} -> {label, Postgres.advisory_lock_key(parts)} end)

      assert map_size(keys) == length(Enum.uniq(Map.values(keys)))
    end

    test "do not collide when part boundaries shift" do
      refute Postgres.advisory_lock_key([@store_domain, "tenant_a", "bc"]) ==
               Postgres.advisory_lock_key([@store_domain, "tenant_ab", "c"])

      refute Postgres.advisory_lock_key([@checkpoint_domain, "public", "default", "reactor"]) ==
               Postgres.advisory_lock_key([@checkpoint_domain, "public", "defaultreactor", ""])
    end

    test "are stable across the signed bigint range, so every node derives the same key" do
      assert 4_058_807_053_573_865_758 ==
               Postgres.advisory_lock_key([
                 @store_domain,
                 "postgres_store_test_schema",
                 "default"
               ])

      assert 6_644_676_092_048_313_026 ==
               Postgres.advisory_lock_key([@checkpoint_domain, "public", "default", "reactor"])

      assert -4_338_057_100_084_004_075 ==
               Postgres.advisory_lock_key([@store_domain, "public", "b"])
    end
  end
end
