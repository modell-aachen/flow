...
**A dispatch normalises its query once, because the read is a value.** `Store.read/3` runs `Query.new/1` and returns a `Store.Read` — the events, the normalised query that selected them, and `last_position`. `AppendCondition.for_read/1` builds the condition straight from those fields, and `AppendCondition.new/1` passes an already-built condition through, so `Store.append/3` does not rebuild what `for_read/1` made. `Query.Item.new/1` serialises module-atom types into the store's string form, so `Item.types` is always the store's representation and there is no separate serialise step anywhere.
