...
**A sync reactor is awaited by Flow, not executed by it.** `sync?/1` means "the dispatch does not return until this run's checkpoint has passed the events it appended", so an engine may enqueue a sync run like any other as long as the job row is durable. `Consistency` (built at dispatch entry, awaited after the commit in `Application.confirm/3`) polls `Store.checkpoint/2` until every sync reactor's checkpoint reaches its own target, then raises `ConsistencyTimeoutError`. The target is **per reactor** — the highest appended position matching that reactor's filter, via `Reactor.matches?/2` on the deserialised envelopes, with reactors matching nothing dropped — because a checkpoint stays put on events the filter skips, so one global "highest appended position" would hang on every dispatch whose last event a sync reactor ignores. That is why `Consistency` carries the sync reactors' `%Reactor{}` declarations rather than their names. It is position-based because a concurrent dispatch's run may be the one that consumes these events under the per-`(prefix, context, name)` lock, leaving this dispatch's job to no-op, so job state would be the wrong thing to watch. An engine executing runs inline therefore needs no changes: its checkpoint committed with the events, so the first poll confirms. The one case that cannot be awaited is a dispatch nested in a caller's transaction — nothing outside it can see the events or the job row — which is why `Application.dispatch/3` asks `Store.in_transaction?/1` *before* opening its own transaction and `ReactorRun.inline?/1` (`nested and sync?`) tells the engine to execute that run inline instead.
