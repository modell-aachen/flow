...
What the dispatch waits on is the reactor's **checkpoint**: it returns once the checkpoint of every synchronous reactor stands at or past the highest position the dispatch appended. Deliberately not the state of whatever the engine enqueued — a reactor's events are consumed one run at a time, so a concurrent dispatch's run may be the one that processes these events and leave this dispatch's own run with nothing to do, and a job that no-ops is not a reactor that failed to catch up. The checkpoint is the store's own record of how far the reactor got, so it holds whoever advanced it.
